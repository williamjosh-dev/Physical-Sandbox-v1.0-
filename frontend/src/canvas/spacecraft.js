...
export default function Spacecraft({ scene, trajectory, currentFrame, blueprint }) {
    ...
    useEffect(() => {
        ...
        activeBlueprint.forEach((part) => {
            ...
            if (part.shape === 'cylinder')
                geo = new THREE.CylinderGeometry(part.scale[0], part.scale[0], part.scale[1], 16);
            else if (part.shape === 'box')
                geo = new THREE.BoxGeometry(...part.scale);
            else if (part.shape === 'sphere')
                geo = new THREE.SphereGeometry(part.scale[0], 16, 16);
            else
                geo = new THREE.ConeGeometry(part.scale[0], part.scale[1], part.scale[2]);
    ...
    useEffect(() => {
        if (!trajectory || trajectory.length === 0 || !groupRef.current)
            return;
    ...
    useEffect(() => {
        if (!trajectory || trajectory.length === 0 || !lineRef.current)
            return;
