...
export function generateCustomShader(mathFormula: string): string {
  return `
    uniform float uTime;
    uniform float uSpeed;
...
export class GPGPUSimulator {
  ...
  constructor(renderer: THREE.WebGLRenderer, size: number = 128, initialShader: string = DEFAULT_POSITION_SHADER) {
    this.size = size;
